...
int main(int argc, char *argv[]) {
      ...
      double **R, *t, **coords1,**coords2, rmsd, **coords, ***Ra, **ta, tt;
        ...
        /* Read options from comman line and set number of threads*/
        ...
        srand48(1);
        /* reads the pdb file and make structure */
...
//printf("%s has %i df\n", defs.res[i],defs.n_tors[i]);


        cra = calloc(MAX_N_SUP_AT, sizeof(struct Cra));

/* Elementary check that models contain the same number of atoms and residues */
...
/* qui ci va il parsing di s1 */

parse(cra,&ncra,flag_par.s1);
//printf("%i\n", ncra);
//for(i=0; i< ncra; i++)
//printf("%c %5i %5s\n", cra[i].c, cra[i].r, cra[i].a);
       for(k=0,l=0;k<ncra;k++)
       cra[k].i = -1;


/* superpose structures on the first if needed */ 
...
parse(cra,&ncra,flag_par.s2);
    ...
    for(i=system.models[0].beg, l=0; i<= system.models[0].end; i++)
       ...
       coords1[l++] = system.atoms[cra[k].i].coor;
       nsupat = l;
// metto le coords alle coordinate del 1, non verranno piu' usate
...
/* Don't need system structure anymore */
    free(system.atoms); 
...
//c = 3.0*log(M_PI/b) - log(12) + 0.5722 + log( (double) counts);
...
void check_cmd_line(int argc, char *argv[], struct Flag_par *flag_par)
{
	...
	{
	printf("Usage:\n"); 
	printf("./trent pdb_infile def_infile outfile -s1 \"string1\" -s2 \"string2\" [Options]\n"); 
...
void make_system(struct System *system)
{
	...
	n_atoms = system->n_atoms;
        ...
        /*** check how many models, segments, chain, residues ***/
...
/*       	 		if (sscanf("%s", atoms[i].segid) == 0) 
        ...
        //printf("3: %i %s\n", i, (*system).atoms[i].at_name);
					...
					strcpy(system->residues[iresidue].seg_name, system->atoms[i].segid);
					system->residues[iresidue].beg = i;
				/*	residues[iresidue].res_type = restyp(system->atoms[i].res_name); */
					if (iresidue != 0) system->residues[iresidue - 1].end = i-1;
        //printf("Sono qui: res_n: %i prev: %i\n", system->atoms[i].res_n,res_n);
        //printf("5: %i %s\n", i, (*system).system->atoms[i].at_name);
					}
					else if(system->atoms[i].res_ins != res_ins) 
					{
					iresidue++;
...
/*     *rmsd = sqrt((innerprod - (2.0 * lambdamax))/(double) len); */
/*    for(i=0; i<4; i++)
      v[i] = ((double) drand48() - 0.5);
*/
    for(i=0; i<4; i++)
    for(j=0; j<4; j++)
        M_copy[i][j] = M[i][j];
...
/*   for(i=0; i<4; i++)
    ...
    max =0.0;
    for(j=0; j<4; j++)
     max = max+M_copy[i][j]*ev[j];
    printf("Mev = lambda * ev %i %12.7f %12.7f\n", i, max, lambdamax * ev[i]);
    } 
    for(j=0; j<4; j++)
    printf("ev %i = %12.7f\n", i, ev[i]);
*/
...
int i;
double t = 0.0;
//for(i = 0; i<4; i++) t = t + q[i]*q[i];
//t = sqrt(t);
//for(i = 0; i<4; i++) q[i]=q[i]/t;
...
//for(i = 0,t=0; i<3; i++) t = t + R[0][i]*R[0][i];
//printf("R[0] = %lf\n", t);
//for(i = 0,t=0; i<3; i++) t = t + R[1][i]*R[1][i];
//printf("R[1] = %lf\n", t);
//for(i = 0,t=0; i<3; i++) t = t + R[2][i]*R[2][i];
//printf("R[2] = %lf\n", t);
}
...
j = k = 0;
while(k == 0 && s[j] == ' ') j++; // tolgo gli spazi all'inizio
for(; j< (int) strlen(s); j++,k++) // tolgo gli spazi in mezzo
{
if(s[j] == ' ') 
{
s[k++]=s[j];
...
//printf("%i\n",(int) strlen(punct));
k = 0;
for(i = 0; i < ntok - 1;) // l'indice dei tok e della punteggiatura vanno assieme
{
//printf("all'inizio tok %i\n",i);
l = -1;
// leggere chain tok[i] contiene le catene
strcpy(c,tok[i++]);
nc=strlen(c);
if(punct[i-1] != ':') 
 {
